...
use crate::utility::{SizedRead, SizedReadExt};
...
impl SizedRead for BlockHeader {
	const SIZE: usize = 16;
}
...
pub async fn read_block<R: AsyncRead + AsyncSeek + Unpin>(
	...
	let block_header = BlockHeader::read_async(reader)
		.await
...
impl<'a, R: AsyncRead + AsyncSeek + Unpin> BlockPayload<'a, R> {
	...
	pub fn new(reader: &'a mut R, input_size: u32, output_size: u32) -> Self {
		...
		let block_reader = match input_size > MAX_COMPRESSED_BLOCK_SIZE {
			true => Either::Left(reader.tak(output_size.into())),
